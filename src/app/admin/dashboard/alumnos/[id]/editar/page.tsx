...
export default function EditarAlumnoPage() {
    ...
    const fetchAlumno = async () => {
      try {
        ...
        const otrasClasesData = otrasClasesResponse.ok ? await otrasClasesResponse.json() : []
          ...
          // Manejar fechaCobro que puede ser null
          const fechaCobroStr = alumno.fechaCobro 
            ? new Date(alumno.fechaCobro).toISOString().split('T')[0] 
            : ''
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Primero actualizar los datos básicos del alumno
      const response = await fetch(`/api/alumnos/${alumnoId}`, {
        method: 'PUT',
        headers: {
      ...
      // Luego guardar las otras clases
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Sección 5: Otros Pagos (Solo lectura) */}
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          <div className="bg-gradient-to-r from-gray-50 to-slate-50 px-6 py-4 border-b border-gray-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">Solo lectura - Próximamente</p>
              </div>
            </div>
          </div>
          <div className="p-6">
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
              <p className="text-sm text-gray-600 text-center">
                📝 Esta sección estará disponible próximanente para registrar otros tipos de pagos.
              </p>
            </div>
          </div>
        </div>

        {/* Sección 6: Registro y Estado */}
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Fecha próxima de cobro al alumno</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-end">
          <button
            ...
            className="px-6 py-3 bg-red-600 text-white text-sm font-semibold rounded-xl hover:bg-red-700 transition-all duration-200 shadow-lg hover:shadow-xl flex items-center justify-center gap-2"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
            </svg>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
            ...
            <div className="p-6">
              <p className="text-sm text-gray-600 mb-6">
                Se eliminarán todos los datos asociados al alumno, incluyendo sus pagos y registros. Esta acción es permanente.
              </p>
              <div className="flex flex-col sm:flex-row gap-3 justify-end">
                <button
                  onClick={() => setShowDeleteModal(false)}
                  className="px-6 py-2.5 border border-gray-200 text-gray-700 text-sm font-semibold rounded-xl hover:bg-gray-50 transition-all duration-200"
                >
                  Cancelar
                </button>
                <button
                  ...
                  disabled={isDeleting}
                  className="px-6 py-2.5 bg-red-600 text-white text-sm font-semibold rounded-xl hover:bg-red-700 transition-all duration-200 shadow-lg flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {isDeleting ? (
                    <>
